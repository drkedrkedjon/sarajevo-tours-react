export default [
  {
    id: "1",
    title: "Guia de pistas en Jahorina",
    subtitle: "Guía GPS completa. Todo incluido",
    hero: {
      heroTitle: "Jahorina",
      heroText: "Olympic mountain with 35km of ski slopes, 31.000 skiiers/h",
    },
    summary:
      "Hoteles, apartamentos o acogedoras casas a pie de pistas. O si tiene un automóvil, quédese en Sarajevo, ya que Jahorina está a solo 24 km de distancia. Tenemos todas las respuestas para ti mientras te preparas para unas vacaciones.",
    description:
      '¿A qué hora levantarse y meter el trasero en el coche? Cómo llegar allí si no tiene coche, dónde aparcar el coche si tiene uno y dónde alquilar un alquiler pirata agradable, sin complicaciones y totalmente asegurado". ¿Cuál es el mejor telesilla para empezar el día (y aparcar el coche a 20 m cerca del telesilla y del quiosco de forfaits) y cómo llegar a las mejores pistas en poco tiempo disfrutando de las primeras rondas antes de que lleguen las escuelas de esquí? ¿mejor si hace calor?, ¿qué hacer si hay niebla, dónde aparcar el coche... a qué pendiente ir?, si está ocupado, cómo avanzar legalmente más rápido en la cola del telesilla que otros esquiadores, lado izquierdo o derecho lado? Tenemos toda la información para todos los remontes lista para usted. ¿Dónde y qué comer, dónde tomar un té? ¿Qué servicio de esquí es mejor? Tenemos respuestas a todas esas preguntas. ¿Necesita más? Vacaciones de Bosnia y países vecinos así que eligió el día correcto sin grandes multitudes. Imágenes, videos, listas de precios, explicación de la comida... ¡ESTÉ tan listo e inteligente COMO ESQUIADOR LOCAL EN NINGÚN TIEMPO!',
    price: "25",
    category: "esqui",
    location: "Jahorina",
    imgUrl: "/assets/images/products/1-1.jpg",
    imgUrl2: "/assets/images/products/1-2.jpg",
  },
  {
    id: "2",
    title: "Guia de alojamientos en Jahorina",
    subtitle: "Guía GPS completa.Todo incluido",
    hero: {
      heroTitle: "Jahorina",
      heroText: "Choose the best, hotel, bungalow, on slopes or down in town",
    },
    summary:
      "¿Cuál es el mejor telesquí para empezar el día (y aparcar el coche a 20 m cerca del telesilla y del quiosco de forfaits) y cómo llegar a las mejores pistas en poco tiempo disfrutando de esas primeras rondas antes de que lleguen las escuelas de esquí? ¿Qué pistas aguantan mejor si hace calor? ¿Qué hacer si hay niebla, dónde aparcar el coche... en qué pendiente ir?",
    description:
      "Jahorina tiene principalmente esquí intermedio en sus pistas sin árboles. Jahorina se utilizó como sede de esquí en los 14.º Juegos Olímpicos de invierno y está cerca de Sarajevo (a solo 20 km). 40 km de pistas cuentan con un eficiente sistema de elevación. Hay una operación de esquí nocturno y un apres-ski bastante bueno. Jahorina tiene un récord de nieve confiable con una temporada larga y una profundidad de nieve promedio de 106 cm para febrero y hasta 3 m en algunas temporadas. Campo a través y biatlón en Dvorista, 4 km por encima de la estación de esquí Jahorina.",
    price: "25",
    category: "esqui",
    location: "Jahorina",
    imgUrl: "/assets/images/bike-card.jpg",
  },
  {
    id: "3",
    title: "Obtener ambas guias de Jahorina con el 20% de descuento",
    subtitle: "Obtener ambos GPS. Todo incluido",
    summary:
      "Obtenga ambas guías de la montaña Jahorina con un descuento especial del 25% sobre el precio total combinado",
    description: "BlaBla",
    price: "40",
    category: "esqui",
    location: "Jahorina",
    imgUrl: "/assets/images/bike-card.jpg",
  },

  {
    id: "4",
    title: "Guia de pistas en Bjelašnica",
    subtitle: "Guía GPS completa.odo incluido",
    summary:
      "Hoteles, apartamentos o acogedoras casas a pie de pistas. O si tiene un automóvil, quédese en Sarajevo, ya que Jahorina está a solo 24 km de distancia. Tenemos todas las respuestas para ti mientras te preparas para unas vacaciones.",
    description:
      "Durante los Juegos Olímpicos de Invierno de 1984, el monte Bjelasnica (Bjelašnica) acogió competiciones de esquí alpino de Man, mientras que el monte Igman (a 10 minutos en coche de Bjelašnica) fue un punto de encuentro para los esquiadores nórdicos: biatlón de fondo. Bjelašnica es la novena montaña más alta de Bosnia y Herzegovina, ¡y puedes esquiar desde su cima! Las pistas terminan a 1270m sobre el nivel del mar. Casi todas las instalaciones construidas para los Juegos Olímpicos de Invierno '84 fueron devastadas durante la guerra reciente, por lo que todas las instalaciones actuales son de nueva construcción. Igman está a solo 10 minutos en coche al oeste de Bjelašnica y representa un lugar perfecto para los amantes de la diversión a campo traviesa o cuesta abajo con niños. Hay una pista de fondo de 20 km en Veliko Polje, mientras que en Malo Polje se puede encontrar un lugar de ensueño para principiantes y niños. Justo al lado de la instalación de saltos de esquí que no funciona, hay un telesilla doble con una pendiente roja/azul y un tramo negro. Además, hay tres ascensores para bebés instalados en la suave pendiente de 100 metros de largo. Mt.Igman es el segundo lugar más rico de Europa por el porcentaje de ozono en el aire.",
    price: "20",
    category: "esqui",
    location: "Bjelasnica",
    imgUrl: "/assets/images/bike-card.jpg",
  },
  {
    id: "5",
    title: "Guia de alojamientos en Bjelašnica",
    subtitle: "Guía GPS completa.Todo incluido",
    summary:
      "¿Cuál es el mejor telesquí para empezar el día (y aparcar el coche a 20 m cerca del telesilla y del quiosco de forfaits) y cómo llegar a las mejores pistas en poco tiempo disfrutando de esas primeras rondas antes de que lleguen las escuelas de esquí? ¿Qué pistas aguantan mejor si hace calor? ¿Qué hacer si hay niebla, dónde aparcar el coche... en qué pendiente ir?",
    description:
      "Las pistas de “Babin Do” son una de las mejores pistas de esquí europeas. Esta estación de esquí cuenta con ocho pistas de esquí y cinco senderos para niños, 3 telesillas y 5 remontes, que conducen a la cima de la montaña Bjelašnica, que se encuentra a 2067 metros sobre el nivel del mar.Partes del sendero están iluminadas con luces de competencia de alta calidad para el esquí nocturno; El centro de esquí también tiene instalado un sistema de nieve artificial.La modernización del transporte vertical, además del telesilla de 6 plazas, también incluye el telesilla de 4 plazas desde “Međustanica” (estación cruzada) hasta la cima de la montaña Bjelašnica. El alquiler y el servicio de esquí se encuentran cerca de la casa de destino Benneton. “Babin Do” dispone de un amplio parking con más de 1.200 plazas de aparcamiento.",
    price: "20",
    category: "esqui",
    location: "Bjelasnica",
    imgUrl: "/assets/images/bike-card.jpg",
  },
  {
    id: "6",
    title: "Obtener ambas guias de Bjelašnica con el 20% de descuento",
    subtitle: "Obtener ambos GPS. Todo incluido",
    summary:
      "Obtenga ambas guías de la montaña Bjelašnica con un descuento especial del 25% sobre el precio total combinado",
    description: "BlaBla",
    price: "30",
    location: "Bjelasnica",
    category: "esqui",
    imgUrl: "/assets/images/bike-card.jpg",
  },
];
